    ...
    >
      ...
      <div className="flex min-h-full items-center justify-center p-4 sm:p-8 pointer-events-none">
        ...
        >
          {/* Close button */}
          <button
            className="absolute top-4 right-4 text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-full w-10 h-10 flex items-center justify-center transition-all focus:outline-none focus:ring-2 focus:ring-slate-400 z-30 bg-white/70 backdrop-blur-md shadow-sm border border-slate-200"
            onClick={onClose}
          ...
          {/* Image Section */}
          <div className="w-full md:w-[60%] bg-slate-50 flex items-center justify-center p-6 sm:p-8 relative overflow-hidden">
            {/* Blurred preview as placeholder */}
            <img
              src={image.previewURL}
              alt=""
              className={`w-full max-h-[50vh] md:max-h-[70vh] object-contain rounded-xl transition-opacity duration-500 ${isHdLoaded ? 'opacity-0 absolute' : 'opacity-100'}`}
            ...
            <img
              ...
              className={`w-full max-h-[50vh] md:max-h-[70vh] object-contain rounded-xl shadow-lg transition-opacity duration-500 ${isHdLoaded ? 'opacity-100' : 'opacity-0 absolute'}`}
            />
          </div>

          {/* Info Panel */}
          <div className="w-full md:w-[40%] p-6 sm:p-8 flex flex-col bg-white overflow-y-auto max-h-[85vh] z-10 border-l border-slate-100">
            {/* User Info */}
            ...
            {/* Stats */}
            <div className="mb-6">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Detailed Info</h3>
              <div className="space-y-1">
                <div className="flex justify-between items-center p-3 rounded-xl hover:bg-slate-50 transition-colors">
                  ...
                  <span className="font-bold text-slate-700 text-lg">{image.likes.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center p-3 rounded-xl hover:bg-slate-50 transition-colors">
                  <span className="text-slate-500 font-medium flex items-center gap-2">
                  ...
                  <span className="font-bold text-slate-700 text-lg">{image.views.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center p-3 rounded-xl hover:bg-slate-50 transition-colors">
                  <span className="text-slate-500 font-medium flex items-center gap-2">
                  ...
                  <span className="font-bold text-slate-700 text-lg">{image.downloads.toLocaleString()}</span>
                </div>
              </div>
            </div>
            ...
            {/* Resolution */}
            <div className="mb-6">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Resolution</h3>
              <div className="px-3 py-2 bg-slate-50 rounded-xl text-sm text-slate-600 font-medium inline-block">
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {image.tags.split(', ').map(tag => (
                  <span key={tag} className="px-3 py-1.5 bg-slate-100 text-slate-700 hover:bg-blue-50 hover:text-blue-700 rounded-lg text-xs font-semibold tracking-wide transition-colors cursor-default">
                    {tag.trim()}
                  </span>
                ))}
              </div>
